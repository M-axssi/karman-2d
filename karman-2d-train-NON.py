...
class NON_Dataset:
    def __init__(self, domain, num_sim, dirpath, begin_step, num_step, batch_size, resolution, need_downsample=True,
        ...
        if need_step:
            for j, asim in enumerate(self.sim_path):
                ...
                for i in range(num_step):
                    if not os.path.isfile(self.FileForOneStep(self.density_path[j][i])):
                        d = phi.field.read(file=self.FileForDownsample(self.density_path[j][i]))
                        v = phi.field.read(file=self.FileForDownsample(self.velocity_path[j][i]))
                        d, v = simulator.step(d, v, re=self.Res[asim], res=self.resolution)
                        phi.field.write(field=d, file=self.FileForOneStep(self.density_path[j][i]))
                        print("Write {}".format(self.FileForOneStep(self.density_path[j][i])))
    ...
    def Get_Data(self, steps=1, type="Native"):
        ...
        density = [
            [
                Data[self.sim_path[self.cur_batch * self.batch_size + j]][self.cur_step + k][0]
                for j in range(self.batch_size)
            ] for k in range(steps)
        ]
        velocity_y = [
            [
                Data[self.sim_path[self.cur_batch * self.batch_size + j]][self.cur_step + k][1]
                for j in range(self.batch_size)
            ] for k in range(steps)
        ]
        velocity_x = [
            [
                Data[self.sim_path[self.cur_batch * self.batch_size + j]][self.cur_step + k][2]
                for j in range(self.batch_size)
            ] for k in range(steps)
        ]
    ...
    def Get_tensor(self, steps=1, type="Native"):
        ...
        D = [
            self.domain.scalar_grid(math.tensor(nD[k], batch('batch'), spatial('y,x')))
            for k in range(steps)
        ]
        V = [
            self.domain.staggered_grid(
                math.stack(
                    [
...
parser.add_argument('--output', type=str, default='./model/NON.pth', help="Path for output model")
...
for i in range(params['epoch']):
    ...
    for j in range(num_batch):
        ...
        while 1:
            count = count + 1
            flag1, _, r_D, r_V = RefDataset.Get_tensor(2, 'Native')
            flag2, Res, _, s_V = RefDataset.Get_tensor(1, 'One step')
            if not (flag1 and flag2):
                break
            loss = train_step(Res, s_V[0], r_V[1], r_D[1], domain)
            total_loss += loss
