...
def get_error(t_Re, s_D, s_V, r_V, domain):
    ...
    loss_velocity_x = loss(
        corr_velocity.vector['x'].values.native(('batch', 'y', 'x')),
        r_V.vector['x'].values.native(('batch', 'y', 'x'))
    )
    loss_velocity_y = loss(
        corr_velocity.vector['y'].values.native(('batch', 'y', 'x')),
        r_V.vector['y'].values.native(('batch', 'y', 'x'))
    )
    total_loss = loss_velocity_x + loss_velocity_y
    return total_loss, sour_state[0], corr_velocity
...
with torch.no_grad():
    for i in range(params['sim']):
        ...
        for j in range(params['steps'] - 1):
            ...
            one_error, density, velocity = get_error(Re, density, velocity, ref_velocity, domain)
